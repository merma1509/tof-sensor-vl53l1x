...
REM Compile i2c_driver
echo Compiling i2c_driver.c...
if defined ARM_GCC_PATH (
    "%ARM_GCC_PATH%\arm-none-eabi-gcc.exe" %CFLAGS% %INCLUDES% -c "%PROJECT_DIR%src\i2c_driver.c" -o "%BUILD_DIR%\i2c_driver.o"
) else (
    arm-none-eabi-gcc.exe %CFLAGS% %INCLUDES% -c "%PROJECT_DIR%src\i2c_driver.c" -o "%BUILD_DIR%\i2c_driver.o"
)

if not exist "%BUILD_DIR%\i2c_driver.o" (
    echo [ERROR] Failed to compile i2c_driver.c
    pause
    exit /b 1
)
echo [SUCCESS] i2c_driver.c compiled
