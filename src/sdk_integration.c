 ...
 * @brief SDK integration implementation for VL53L1 and GD32E230
 */
...
void sdk_gpio_init(void) {
    ...
    // Configure I2C pins (PA6/PA7)
    gpio_mode_set(GPIOA, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, GPIO_PIN_6 | GPIO_PIN_7);
...
uint8_t sdk_vl53l1_init(void) {
    // Initialize VL53L1 using API
    VL53L1_ERROR status = VL53L1_SensorInit(VL53L1_I2C_ADDRESS);
    
    if (status == VL53L1_ERROR_NONE) {
        g_sdk.vl53l1_initialized = 1;
...
uint8_t sdk_vl53l1_get_distance(uint16_t *distance) {
    if (!g_sdk.vl53l1_initialized || !distance) {
        return 0;
    }
    
    VL53L1_ERROR status = VL53L1_GetDistance(VL53L1_I2C_ADDRESS, distance);
...
uint8_t sdk_vl53l1_set_mode(uint8_t mode) {
    if (!g_sdk.vl53l1_initialized) {
        return 0;
    }
    
    VL53L1_ERROR status = VL53L1_SetDistanceMode(VL53L1_I2C_ADDRESS, mode);
...
uint8_t sdk_vl53l1_set_timing(uint32_t timing) {
    ...
    VL53L1_ERROR status = VL53L1_SetTimingBudgetInMs(VL53L1_I2C_ADDRESS, timing);
...
uint8_t sdk_vl53l1_calibrate(void) {
    ...
    // Use VL53L1 calibration API
    VL53L1_ERROR status = VL53L1_StartTemperatureUpdate(VL53L1_I2C_ADDRESS);
    
    return (status == VL53L1_ERROR_NONE) ? 1 : 0;
}
