 ...
 * @brief UART command interface implementation for VL53L1 control
 */
...
void uart_send_string(const char *str) {
    while (*str) {
        ...
        while (!(*(volatile uint32_t*)0x40013800 & (1 << 7))); // USART_STAT_TBE
        *(volatile uint32_t*)0x40013804 = *str; // USART_DATA
        str++;
    }
}
...
void uart_process_command(char *cmd) {
    ...
    if (strcmp(cmd, CMD_START) == 0) {
        if (!measuring) {
            // Start VL53L1 ranging using I2C
            // Write to start register
            uint8_t start_result = i2c_write_register(0x010, 0x01); // VL53L1_SYSRANGE_START
            if (start_result == 0) {
                measuring = 1;
        ...
        if (measuring) {
            // Stop VL53L1 ranging
            uint8_t stop_result = i2c_write_register(0x010, 0x00); // VL53L1_SYSRANGE_STOP
            if (stop_result == 0) {
                measuring = 0;
                uart_send_response(RESP_OK);
        ...
        // Step 1: Soft reset
        uint8_t reset_result = i2c_write_register(0x0000, 0x00); // VL53L1_SOFT_RESET
        delay_ms(10);
        
        // Step 2: Clear interrupt
        uint8_t clear_result = i2c_write_register(0x0086, 0x01); // VL53L1_SYSTEM_INTERRUPT_CLEAR
        delay_ms(5);
        
        // Step 3: Set measurement timing budget to 100ms for calibration
        uint8_t timing_result = i2c_write_register(0x000A, 0x00); // Low byte
        uint8_t timing_high = i2c_write_register(0x000B, 0x27); // High byte (10000us = 100ms)
        
        // Step 4: Set distance mode to long for calibration
        uint8_t mode_result = i2c_write_register(0x0011, 0x03); // VL53L1_DISTANCE_MODE_LONG
        
        // Step 5: Start ranging for calibration
        uint8_t start_result = i2c_write_register(0x0087, 0x01); // VL53L1_SYSTEM_MODE_START
        
        // Wait for calibration to complete
        delay_ms(100);
        
        // Step 6: Stop ranging
        uint8_t stop_result = i2c_write_register(0x0087, 0x00); // VL53L1_SYSTEM_MODE_STOP
        ...
        i2c_write_register(0x0087, 0x00); // VL53L1_SYSTEM_MODE_STOP
        delay_ms(10);
        
        // Step 2: Set distance mode to short
        uint8_t mode_result = i2c_write_register(0x0011, 0x01); // VL53L1_DISTANCE_MODE_SHORT
        
        // Step 3: Set timing budget to 15ms
        uint8_t timing_low = i2c_write_register(0x000A, 0xB0); // Low byte (15000us)
        uint8_t timing_high = i2c_write_register(0x000B, 0x03); // High byte
        ...
        uint8_t mode_result = i2c_write_register(0x0011, 0x02); // VL53L1_DISTANCE_MODE_MEDIUM
        
        // Step 3: Set timing budget to 50ms
        uint8_t timing_low = i2c_write_register(0x000A, 0x88); // Low byte (50000us)
        uint8_t timing_high = i2c_write_register(0x000B, 0x0C); // High byte
        ...
        uint8_t timing_low = i2c_write_register(0x000A, 0x10); // Low byte (100000us)
        uint8_t timing_high = i2c_write_register(0x000B, 0x86); // High byte
        ...
        if (measuring) {
            ...
            uint8_t distance_high = i2c_read_register(0x0096); // VL53L1_RESULT_DISTANCE_HIGH
            uint8_t distance_low = i2c_read_register(0x0097); // VL53L1_RESULT_DISTANCE_LOW
            ...
            uint8_t mode = i2c_read_register(0x0011); // VL53L1_DISTANCE_MODE
            ...
            uint8_t timing_low = i2c_read_register(0x000A);
            uint8_t timing_high = i2c_read_register(0x000B);
